package zerobase.lecture.ch02.ch02_datastructure.linear_data_test.linked_list;
...
//        연결리스트의 head 가 null 이라면, 가장 맨 처음 head에 노드를 새로 생성해서, head에 data를 넣고 next에 null을 넣는다.
        if(this.head == null){
            // 맨 처음, 새 노드 안에 data 넣는다.
            this.head = new Node(data, null);
        }
        //            노드가 있다면
        // 첫 헤드를 가리키도록 보존하고 cur 이 헤드를 가리킬 수 있도록
        else{
            Node cur = this.head;
            //                next 가 있을 때까지 이동
            while (cur.next != null){
