package zerobase.lecture.w1.ch03.non_linear_data.heap;
// 비선형자료구조 - 힙
// ArrayList 로 최소 힙 구현
...
class MaxHeap{
    ...
    public void insert(int data) {
        ...
        while(cur > 1 && heap.get(cur / 2) < heap.get(cur)){ // ---------------------- MaxHeap 위해 부등호 변경
...
//        가장 위에 올린 데이터가 MaxHeap 기준에 적합한지 판단하기
        int cur = 1;
        while(true) {
//            해당 케이스가 나올때까지 반복문
            int leftIdx = cur * 2;
...
//        위에서 선정한 타겟값 비교
            if(heap.get(cur) > heap.get(targetIdx)){ // ---------------------- MaxHeap 위해 부등호 변경
                break;
            } else {
